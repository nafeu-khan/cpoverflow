...
const Tags = ({ searchParams }: SearchParamsProps) => {
  const [result, setResult] = useState<any>({ tags: [], isNext: false });
  ...
  useEffect(() => {
    const fetchTags = async () => {
      try {
        ...
        if (searchParams.filter) params.append('sort', searchParams.filter);
        ...
        setResult(response); // Direct response, not response.data
      } catch (error) {
        console.error('Error fetching tags:', error);
        setResult({ tags: [], isNext: false });
      } finally {
        setLoading(false);
      }
    };
